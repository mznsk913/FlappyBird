...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    var itemNode:SKNode! //itemNode追加
    let sound = SKAction.playSoundFileNamed("sound.mp3", waitForCompletion: false) //効果音
    ...
    let itemCategory: UInt32 = 1 << 4      // 0...10000 //itemCategory追加
    ...
    func setupItem() {
        ...
        // itemをまとめるノードを作成
            //let item = SKNode() // ここを変更
            let item = SKSpriteNode(texture: itemTexture)
            item.position = CGPoint(x: self.frame.size.width + itemTexture.size().width / 2, y: 500)
        ...
        // itemを表示するノードにitemの作成を無限に繰り返すアクションを設定
